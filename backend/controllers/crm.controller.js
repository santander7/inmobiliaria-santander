...
exports.obtenerLeads = async (req, res) => {
  try {
    const leads = await Solicitud.find().populate('user', 'nombre correo telefono').sort({ createdAt: -1 });
...
exports.actualizarEstadoLead = async (req, res) => {
  try {
    ...
    const leadActualizado = await Solicitud.findByIdAndUpdate(id, { estado }, { new: true }).populate('user', 'nombre correo telefono');
